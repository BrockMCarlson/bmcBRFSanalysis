% load('D:\bmcBRFS_sortedData_Nov23\sortedData_221123_J_bmcBRFS001.mat')

% Plot all condition averages
for cond = 1:20
...
% it looks super wierd
% But LFP_bb is the most beautifully time locked across all electrodes
figure
    ...
    for trialNumber = 1:numTrials
        for channel1 = 1:32
            for channel2 = 1:32
                ...
                lfpGammaData1 = IDX(conditionNumber).LFP_bb{trialNumber, 1}(:, channel1);
                lfpGammaData2 = IDX(conditionNumber).LFP_bb{trialNumber, 1}(:, channel2);
        ...
        subplot(1,2,1)
        plot(-200:800,IDX(conditionNumber).LFP_bb{trialNumber, 1}(:, 10))
        hold on
        plot(-200:800,IDX(conditionNumber).LFP_bb{trialNumber, 1}(:, 11))
        vline(0)
        title('contact 10 and contact 11')
        subplot(1,2,2)
        [coherence, freq] = mscohere(...
            IDX(conditionNumber).LFP_bb{trialNumber, 1}(400:1001, 10),...
            IDX(conditionNumber).LFP_bb{trialNumber, 1}(400:1001, 11),...
        ...
        % Data 1
        subplot(1,2,1)
        plot(-200:800,IDX(conditionNumber).LFP_bb{trialNumber, 1}(:,:))
        vline(0)
        % imagesc Coherence
        subplot(1,2,2)
