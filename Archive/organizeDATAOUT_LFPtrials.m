...
cd(codeDir)
outDir = 'C:\Users\Brock Carlson\Box\Manuscripts\Maier';
...
DATAOUT_ps = cell(size(officLamAssign,1),1);
DATAOUT_ns = cell(size(officLamAssign,1),1);
    ...
    load(fileToLoad)
        ...
        for i = 1:length(ch)
            ...
            for trl = 1:length(IDX(prefCondOnFlash).correctTrialIndex)
                array_dichopticAdapted_pref(tm1,i,trl) = abs(IDX(prefCondOnFlash).LFP_bb{trl,1}(tm1,i)); % now we index the cell for the second 800ms
                array_dichopticAdapted_pref(tm2_concat,i,trl) = abs(IDX(prefCondOnFlash).LFP_bb{trl,2}(tm2,i)); % now we index the cell for the second 800ms
            end
            for trl = 1:length(IDX(nullCondOnFlash).correctTrialIndex)
                array_dichopticAdapted_null(tm1,i,trl) = abs(IDX(nullCondOnFlash).LFP_bb{trl,1}(tm1,i)); % now we index the cell for the second 800ms
                array_dichopticAdapted_null(tm2_concat,i,trl) = abs(IDX(nullCondOnFlash).LFP_bb{trl,2}(tm2,i)); % now we index the cell for the second 800ms
            end
        end
        % output is tm, ch, trl
        ...
        % Save this output
        DATAOUT_ps{file} = array_dichopticAdapted_pref; % output is tm, ch, trl
        DATAOUT_ns{file} = array_dichopticAdapted_null;
       ...
       % %  v1Ch = v1Top:v1Btm;
       % % 
       % %  MUAe.ps_PercentC(:,:,file) = ps_PercentC(:,v1Ch);
       % %  MUAe.ns_PercentC(:,:,file) = ns_PercentC(:,v1Ch);
       % % 
