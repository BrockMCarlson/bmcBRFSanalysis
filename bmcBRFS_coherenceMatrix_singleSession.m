...
officLamAssign = importLaminarAssignments("C:\Users\Neuropixel\Box\Manuscripts\Maier\officialLaminarAssignment_bmcBRFS.xlsx", "Sheet1", [2, Inf]);
...
if any(v1Ch > 32) || any(v1Ch < 1)
    warning('skipp session without full column for now')
    disp(sessionLabel)
...
close all
count = 0;
averageCoherenceMatrix = nan(32,32,2);
...
figure;
imagesc(averageCoherenceMatrix(:,:,1));
colormap('jet');
colorbar;
xlabel('Channel');
ylabel('Channel');
...
figure;
imagesc(averageCoherenceMatrix(:,:,2));
colormap('jet');
colorbar;
xlabel('Channel');
ylabel('Channel');
...
% abs(cond9-cond10)
% abs(cond12-cond11)
differenceMatrix = nan(32,32);
differenceMatrix(:,:) = abs(averageCoherenceMatrix(:,:,1) - averageCoherenceMatrix(:,:,2));
