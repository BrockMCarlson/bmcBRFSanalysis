...
DATAOUT_ps = nan(1201,32,size(officLamAssign,1));
DATAOUT_ns = nan(1201,32,size(officLamAssign,1));
...
for file = 1:size(officLamAssign,1)
    ...
    load(fileToLoad)
        ...
        %% Create array of date based on mode of preference for el
        % single timecourse
        ...
        if mode(prefMonoc) == 1
            prefCondOnFlash = 12;
            nullCondOnFlash = 11;
        elseif mode(prefMonoc) == 2
            prefCondOnFlash = 9;
            nullCondOnFlash = 10;
        elseif mode(prefMonoc) == 3
            prefCondOnFlash = 10;
            nullCondOnFlash = 9;
        elseif mode(prefMonoc) == 4
            prefCondOnFlash = 11;
            nullCondOnFlash = 12;
        end
    
        % pre allocate
        array_dichopticAdapted_pref = nan(1201,length(ch),length(IDX(prefCondOnFlash).correctTrialIndex));
        array_dichopticAdapted_null = nan(1201,length(ch),length(IDX(nullCondOnFlash).correctTrialIndex));

        % convert cell to array
        for trl = 1:length(IDX(prefCondOnFlash).correctTrialIndex)
            array_dichopticAdapted_pref(:,:,trl) = IDX(prefCondOnFlash).CSD_bb{trl,2}(:,ch); % now we index the cell for the second 800ms
        end
        for trl = 1:length(IDX(nullCondOnFlash).correctTrialIndex)
            array_dichopticAdapted_null(:,:,trl) = IDX(nullCondOnFlash).CSD_bb{trl,2}(:,ch); % now we index the cell for the second 800ms
        end
       ...
       % %  v1Ch = v1Top:v1Btm;
       % % 
       % %  CSD_bb.ps_PercentC(:,:,file) = ps_PercentC(:,v1Ch);
       % %  CSD_bb.ns_PercentC(:,:,file) = ns_PercentC(:,v1Ch);
       % % 
...
cd(outDir)
save('DATAOUT_CSD.mat','DATAOUT_ps','DATAOUT_ns','DATAOUT_numTuned', '-v7.3')
datetime